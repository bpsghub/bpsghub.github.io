...
Here's the thing: when I checked the codebase, **Next.js was already on the latest version (15.x)**. React too. The framework didn't need updating.

What the client perceived as an outdated framework was really just a broken website caused by DNS. Sometimes "please update our framework" actually means "please make our website work again."

I still did a clean rebuild of the container, updated WordPress plugins (Yoast SEO and the active theme), and removed the SSL bypass — so the codebase came out cleaner and more secure than before.
